  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: iconColor + '22' }}>
...
export default function DashboardClient({
  ...
  const recentActivity = [
    ...expenses.slice(0, 5).map((e) => ({
      ...
      description: `Kosten: ${e.description} — ${formatCurrency(Number(e.amount))}`,
      time: e.created_at,
    })),
    ...allTasks.filter((t) => (t.status === 'voltooid' || t.status === 'done') && t.completed_at).slice(0, 5).map((t) => ({
    ...
    >
      ...
      <div className="mb-7">
        ...
        {activeProject && (
          <p className="text-sm mt-1" style={{ color: '#6B7280' }}>
            Actief project:{' '}
            <Link href={`/projects/${activeProject.id}`} className="font-semibold" style={{ color: '#288760' }}>
              {activeProject.name}
            </Link>
          </p>
        )}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        {activeProject && budget > 0 && (
          <div
            className="rounded-2xl p-6 bg-white border transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-base font-bold mt-1" style={{ color: '#288760' }}>{formatCurrency(Math.max(budget - totalExpenses, 0))}</p>
              </div>
            </div>
            {/* Progress bar */}
            <div className="w-full rounded-full h-3 mb-2 overflow-hidden" style={{ backgroundColor: '#E5E7EB' }}>
              <div
                ...
                style={{ width: `${budgetPercentage}%`, background: `linear-gradient(90deg, ${budgetColor}cc, ${budgetColor})` }}
              />
            </div>
        ...
        {/* No active project */}
        {!activeProject && (
          <div className="rounded-2xl p-6 bg-white border text-center transition-all duration-200 hover:-translate-y-0.5"
            style={{ borderColor: '#E5E7EB', boxShadow: '0 2px 16px rgba(0,0,0,0.07)' }}>
            ...
            <ul className="space-y-2">
              {todayTasks.map((task) => {
                ...
                return (
                  <li
                    key={task.id}
                    className="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-gray-50 transition-all duration-150 cursor-pointer"
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        borderColor: done ? '#288760' : '#D1D5DB',
                        backgroundColor: done ? '#288760' : 'white',
                        boxShadow: done ? '0 0 0 3px rgba(40,135,96,0.15)' : 'none',
                      }}
                    >
                      <svg
                        className="w-3.5 h-3.5 text-white transition-all duration-200"
                        style={{ opacity: done ? 1 : 0, transform: done ? 'scale(1)' : 'scale(0.5)' }}
        ...
        {/* ── Recente activiteit ── */}
        <div
          className="rounded-2xl p-6 bg-white border lg:col-span-2 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
          style={{ borderColor: '#E5E7EB', boxShadow: '0 2px 16px rgba(0,0,0,0.07)' }}
        >
          <h2 className="text-base font-semibold mb-5" style={{ color: '#1A1A1A' }}>Recente activiteit</h2>
          {recentActivity.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-sm" style={{ color: '#6B7280' }}>Nog geen activiteit. Voeg kosten of taken toe.</p>
            </div>
          ) : (
            <ul className="space-y-1">
              {recentActivity.map((activity, idx) => {
                const s = activityStyle[activity.type] ?? activityStyle.expense;
                ...
                return (
                  <li key={activity.id} className="flex items-start gap-0">
                    ...
                    <div className="flex flex-col items-center mr-3 shrink-0">
                      ...
                      {!isLast && (
                        <div className="w-px flex-1 my-1" style={{ backgroundColor: '#E5E7EB', minHeight: '16px' }} />
                      )}
                    </div>
                    {/* Content */}
                    <div className="flex-1 min-w-0 py-1.5 px-3 rounded-xl hover:bg-gray-50 transition-colors cursor-default"
                      style={{ marginBottom: isLast ? 0 : '4px' }}>
