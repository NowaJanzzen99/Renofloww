...
function DonutGauge({ percentage, color }: { percentage: number; color: string }) {
  ...
  return (
    <svg className="w-12 h-12 sm:w-16 sm:h-16 shrink-0" viewBox="0 0 80 80">
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <div className="min-w-0">
            <p className="text-xs sm:text-lg font-black leading-tight whitespace-nowrap" style={{ color: '#1A1A1A' }}>{budget > 0 ? formatCurrency(totalExpenses) : '—'}</p>
            <p className="text-[10px] sm:text-xs whitespace-nowrap" style={{ color: '#9CA3AF' }}>{budget > 0 ? `van ${formatCurrency(budget)}` : 'Geen budget'}</p>
          </div>
        </div>
