      ...
      >
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          ...
          {profile && !profile.is_pro && (
            <>
              <style>{`
                @keyframes rf-glass-shimmer {
                  0%   { transform: translateX(-110%) skewX(-20deg); }
                  100% { transform: translateX(340%) skewX(-20deg); }
                }
                .rf-upgrade-compact .rf-glass {
                  animation: rf-glass-shimmer 4s ease-in-out infinite;
                }
              `}</style>
              <Link
                href="/settings#abonnement"
                className="rf-upgrade-compact relative flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg mt-2 overflow-hidden transition-opacity hover:opacity-90"
                style={{ background: '#162d22' }}
              >
                <div
                  className="rf-glass absolute inset-0 pointer-events-none"
                  style={{ width: '30%', background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.07), transparent)' }}
                />
                <svg className="w-3 h-3 shrink-0 relative" fill="currentColor" viewBox="0 0 24 24" style={{ color: 'rgba(255,255,255,0.6)' }}>
                  <path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                </svg>
                <span className="text-xs font-semibold relative" style={{ color: 'white' }}>Upgrade naar Pro</span>
              </Link>
            </>
          )}
        </nav>
