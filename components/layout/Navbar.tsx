...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      {/* Right side — single relative container so both dropdowns align to the right edge */}
      <div className="relative flex items-center gap-2" ref={rightRef}>
        ...
        >
          ...
          {/* Red count badge — only shown when there are unread notifications */}
          {unreadCount > 0 && (
            <span
              className="absolute -top-1 -right-1 min-w-[18px] h-[18px] rounded-full flex items-center justify-center text-white font-black leading-none pointer-events-none"
              style={{ backgroundColor: '#EF4444', fontSize: '10px', padding: '1px 4px', boxShadow: '0 0 0 2px white' }}
            >
        ...
        >
          <div className="w-7 h-7 rounded-full overflow-hidden shrink-0">
            {profile?.avatar_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-white text-xs font-semibold" style={{ backgroundColor: '#288760' }}>
        ...
        {showNotifs && (
          <div
            className="absolute right-0 top-full mt-2 w-80 max-w-[calc(100vw-16px)] rounded-2xl border bg-white z-50"
            style={{ borderColor: '#E5E7EB', boxShadow: '0 8px 32px rgba(0,0,0,0.14)' }}
          >
            <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: '#E5E7EB' }}>
              <span className="text-sm font-semibold" style={{ color: '#1A1A1A' }}>
                Meldingen
                {unreadCount > 0 && (
                  <span className="ml-1.5 text-xs px-1.5 py-0.5 rounded-full text-white font-black" style={{ backgroundColor: '#EF4444' }}>
                    {unreadCount}
                  </span>
                )}
              </span>
            ...
            <div className="max-h-72 overflow-y-auto">
              {notifications.length === 0 ? (
                ...
                notifications.map((notif) => (
                  <button
                    key={notif.id}
                    onClick={() => { markAsRead(notif.id); if (notif.link) router.push(notif.link); setShowNotifs(false); }}
                    className="w-full text-left px-4 py-3 border-b hover:bg-gray-50 transition-colors"
                    style={{ borderColor: '#F3F4F6', backgroundColor: notif.is_read ? 'transparent' : '#F8FAF9' }}
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: notifTypeColor[notif.type] || '#288760' }} />
                      <div className="min-w-0">
                        <p className="text-sm font-medium" style={{ color: '#1A1A1A' }}>{notif.title}</p>
                        {notif.message && <p className="text-xs mt-0.5 line-clamp-2" style={{ color: '#6B7280' }}>{notif.message}</p>}
        ...
        {showUserMenu && (
          <div
            className="absolute right-0 top-full mt-2 w-52 rounded-2xl border bg-white z-50"
            style={{ borderColor: '#E5E7EB', boxShadow: '0 8px 32px rgba(0,0,0,0.14)' }}
          >
            <div className="px-4 py-3 border-b flex items-center gap-3" style={{ borderColor: '#E5E7EB' }}>
              <div className="w-9 h-9 rounded-full overflow-hidden shrink-0">
                {profile?.avatar_url ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-white text-sm font-semibold" style={{ backgroundColor: '#288760' }}>
                    {profile?.name ? profile.name[0].toUpperCase() : profile?.email?.[0]?.toUpperCase() || 'U'}
                  </div>
                )}
              </div>
            ...
            <div className="p-1.5">
              <Link href="/settings" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm hover:bg-gray-50 transition-colors" style={{ color: '#1A1A1A' }}>
              ...
              <button onClick={handleSignOut} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm hover:bg-red-50 transition-colors" style={{ color: '#EF4444' }}>
